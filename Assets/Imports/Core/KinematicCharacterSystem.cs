...
namespace Imports.Core
{
    ...
    public class KinematicCharacterSystem : MonoBehaviour
    {
        ...
        /// <summary>
        /// Normalizes a quaternion, returning identity if the quaternion is invalid
        /// </summary>
        ...
        public static void PreSimulationInterpolationUpdate(float deltaTime)
        {
            ...
            for (int i = 0; i < CharacterMotors.Count; i++)
            {
                ...
                motor.InitialTickRotation = NormalizeQuaternion(motor.TransientRotation);

                motor.Transform.SetPositionAndRotation(motor.TransientPosition, motor.InitialTickRotation);
            }
            ...
            for (int i = 0; i < PhysicsMovers.Count; i++)
            {
                ...
                mover.InitialTickRotation = NormalizeQuaternion(mover.TransientRotation);

                mover.Transform.SetPositionAndRotation(mover.TransientPosition, mover.InitialTickRotation);
...
#pragma warning disable 0162
            ...
            for (int i = 0; i < physicsMoversCount; i++)
            {
                ...
                Quaternion normalizedRotation = NormalizeQuaternion(mover.TransientRotation);

                mover.Transform.SetPositionAndRotation(mover.TransientPosition, normalizedRotation);
                mover.Rigidbody.position = mover.TransientPosition;
                mover.Rigidbody.rotation = normalizedRotation;
            }
...
#pragma warning restore 0162
        ...
        public static void PostSimulationInterpolationUpdate(float deltaTime)
        {
            ...
            {
                PhysicsMover mover = PhysicsMovers[i];
                Quaternion normalizedInitial = NormalizeQuaternion(mover.InitialTickRotation);
                Quaternion normalizedTransient = NormalizeQuaternion(mover.TransientRotation);

                if (mover.MoveWithPhysics)
                {
                    mover.Rigidbody.position = mover.InitialTickPosition;
                    mover.Rigidbody.rotation = normalizedInitial;

                    mover.Rigidbody.MovePosition(mover.TransientPosition);
                    mover.Rigidbody.MoveRotation(normalizedTransient);
                }
                else
                {
                    mover.Rigidbody.position = mover.TransientPosition;
                    mover.Rigidbody.rotation = normalizedTransient;
                }
            }
        }
